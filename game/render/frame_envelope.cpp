...
// framework edit. A disabled build is not an escape hatch — it makes the picture WORSE, which is
...
void FrameEnvelope::produce(Core *c, uint32_t drawEnvAddr, uint32_t dispEnvAddr) {
  ...
  // GP1(05) is issued unconditionally by the guest on every call and is what alternates the
  // scanned- out page between VRAM y=0 and y=256 — the single most picture-critical word in the
  // envelope.
  gpu_gp1(c, disp.displayStartWord());
  // GP1(08)/GP1(07) are issued by the guest only when the DISPENV differs from libgpu's cached
  // copy. The port keeps its OWN cache of the last block it programmed rather than reading
  // libgpu's, for the same reason it recomputes the words instead of copying them: on this leg
  // libgpu's cache is never updated (its PutDispEnv never runs), so reading it would compare
  // against a stale value and silently stop re-programming a mode that had genuinely changed.
...
// DR_ENV at DRAWENV+0x1C; this recomputes the same words from the same DRAWENV and compares.
// Nothing about the picture depends on it — it exists so the arithmetic above is CHECKED against
// libgpu's on every frame of a real boot rather than argued for on paper.
//
