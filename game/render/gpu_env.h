// gpu_env.h — DrawEnv / DispEnv: typed lenses over the two Sony environment blocks this engine
// hands the GPU every frame, plus the PORTED libgpu leaves that turn them into GPU command words.
//
// WHY THESE ARE GAME STATE AND NOT "WHAT THE GPU PRODUCED" (coord/PROTOCOL.md picture rule). A
// DRAWENV/DISPENV is a struct the ENGINE fills in and SUBMITS. Reading it is reading the
// submitter's inputs, which is exactly what the rule requires; the thing it forbids is going the
// other way — recovering state by inverting a GP0 packet, an OT link or a GTE register. Nothing
// here reads any of those. The word builders below are the port of libgpu's own field→word
// functions, so the port computes the same words from the same inputs rather than copying the words
// the guest computed.
//
// ─────────────────────────────────────────────────────────────────────────────────────────────────
// RE PROVENANCE. Every offset and every bit position below comes from the game's own libgpu, read
// out of the image with Ghidra headless:
//
...
//   0x80082000  PutDispEnv(env)  -> GP1(05) display start UNCONDITIONALLY, then (only when the
//   block
...
// from this game's DISPENV (w=512, isinter=0, isrgb24=0, NTSC) is 0x08000002, and the framework's
// own GPU model independently logs `[gpu] display depth -> 15-bit (GP1(08)=08000002, 512x240)` on a
...
public:
  ...
  // The drawing clip rectangle, in VRAM coordinates. This is also the rectangle the background
  // clear covers, which is why the clear needs no separate geometry of its own.
  int clipX() const {
    return mClipX;
  }
  int clipY() const {
  ...
  int ofsX() const {
    return mOfsX;
  } // added to every primitive's XY by the GPU
  int ofsY() const {
    return mOfsY;
  }
  ...
  // The background clear, exactly as FUN_80082770 encodes it. Returns the number of words written
  // to `out` (0 when isbg is clear, 3 otherwise). Two encodings, and the guest chooses between
  // them: a 64-aligned clip becomes a GP0(02) VRAM FILL (which ignores the clip and the draw
  // offset), and anything else becomes a GP0(60) flat rectangle in drawing space. Both are
  // reproduced because the choice is the guest's, not ours.
...
public:
  ...
  int screenX() const {
    return mScreenX;
  } // the analog screen position/size; screenH == 0 means
  int screenY() const {
    return mScreenY;
  } // "libgpu's default height", which is what this game ships
  int screenW() const {
    return mScreenW;
  }
  int screenH() const {
  ...
  // libgpu's CRT timing tables at 0x800B0EFC / 0x800B0F24, which are not RE'd — and this
  // framework's GPU model DISCARDS GP1(06) outright (`case 0x06: break;` in gpu_native.cpp, with a
  // measurement showing the span it carries only ever restates the resolution GP1(08) already
  // gave). So the omission is stated, bounded, and has no picture consequence on this backend. If a
  // consumer ever needs it, the tables are the RE step, not a guessed constant.
