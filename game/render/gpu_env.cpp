...
// offset and bit position; this file is the transcription-free restatement of those five functions in
// terms of named fields.
#include "gpu_env.h"
#include "core.h"
...
int clampToExtent(int v, int extent) {
  ...
  if (s < 0) return 0;
  if (s > extent - 1) return extent - 1;
  return s;
}

// The guest's own NTSC/PAL selector. `tools/ghidra_query.py func 0x8008BF64` decompiles to exactly
// `return DAT_800B393C;` — libgpu's GetVideoMode is a single load of that byte, and PutDispEnv calls
// it to pick both the GP1(08) PAL bit and the GP1(07) scanline constants. This port is the USA
// release (SLUS_008.75) and its boot programs GP1(08)=0x08000002 with no PAL bit — but the value is
// READ rather than assumed, so a PAL image would come out right instead of silently wrong.
constexpr uint32_t kLibgpuVideoMode = 0x800B393Cu;

// libgpu's "reverse video" flag, ORed into GP1(08) as bit 7 by PutDispEnv (`DAT_800b0e2b`). Read for
// the same reason: it is a real field of the submission, and defaulting it would be a guess.
...
void DrawEnv::read(Core* c, uint32_t base) {
  mBase  = base;
  if (!base) return;                      // a null context is a real state at boot; every field stays 0
  mClipX = c->mem_r16(base + 0x00); mClipY = c->mem_r16(base + 0x02);
  mClipW = c->mem_r16(base + 0x04); mClipH = c->mem_r16(base + 0x06);
  mOfsX  = (int16_t)c->mem_r16(base + 0x08); mOfsY = (int16_t)c->mem_r16(base + 0x0A);
  mTwX   = c->mem_r8(base + 0x0C); mTwY = c->mem_r8(base + 0x0E);
  mTwW   = (int16_t)c->mem_r16(base + 0x10); mTwH = (int16_t)c->mem_r16(base + 0x12);
  mTpage = c->mem_r16(base + 0x14);
  mDtd   = c->mem_r8(base + 0x16); mDfe = c->mem_r8(base + 0x17); mIsBg = c->mem_r8(base + 0x18);
  mR = c->mem_r8(base + 0x19); mG = c->mem_r8(base + 0x1A); mB = c->mem_r8(base + 0x1B);
}
...
uint32_t DrawEnv::drawModeWord() const {
  // FUN_800829E0(dfe, dtd, tpage): bit 9 = dither, bit 10 = draw-to-display-area, and the tpage's own
  // bits survive through the 0x9FF mask (which is what drops any bit 9/10 the tpage word carried).
...
uint32_t DrawEnv::textureWindowWord() const {
  ...
  return 0xE2000000u | ((uint32_t)(mTwY >> 3) & 0x1Fu) << 15
                     | ((uint32_t)(mTwX >> 3) & 0x1Fu) << 10
                     | ((uint32_t)(((-mTwH) & 0xFF) >> 3) & 0x1Fu) << 5
...
int DrawEnv::backgroundClearWords(Core* c, uint32_t* out) const {
  ...
  // The guest's own branch: a clip whose X and width are both 64-aligned can use the VRAM FILL, which
  // is faster on hardware and ignores clip/offset; anything else has to be a real rectangle drawn in
  // drawing space, which is why its position is (clip - ofs) rather than clip.
...
void DispEnv::read(Core* c, uint32_t base) {
  mBase = base;
  if (!base) return;
  mDispX = c->mem_r16(base + 0x00); mDispY = c->mem_r16(base + 0x02);
  mDispW = c->mem_r16(base + 0x04); mDispH = c->mem_r16(base + 0x06);
  mScreenX = (int16_t)c->mem_r16(base + 0x08); mScreenY = (int16_t)c->mem_r16(base + 0x0A);
  mScreenW = (int16_t)c->mem_r16(base + 0x0C); mScreenH = (int16_t)c->mem_r16(base + 0x0E);
  mIsInter = c->mem_r8(base + 0x10); mIsRgb24 = c->mem_r8(base + 0x11);
}
...
uint32_t DispEnv::displayModeWord(Core* c) const {
  ...
  if (mIsRgb24) w |= 0x10u;      // bit 4 — 24-bit display colour depth
  if (mIsInter) w |= 0x20u;      // bit 5 — interlace
  if (c->mem_r8(kLibgpuReverse)) w |= 0x80u;   // bit 7 — libgpu's reverse-video flag
  ...
  if (w16 > 0x118) {
    if      (w16 < 0x161) w |= 1;      // 320
    else if (w16 < 0x191) w |= 0x40;   // 368
    else if (w16 < 0x231) w |= 2;      // 512
    else                  w |= 3;      // 640
  ...
  if (!fitsOneField) w |= 0x24u;       // bits 2 + 5 — VRes 480 with interlace
  return w;
}
...
uint32_t DispEnv::verticalRangeWord(Core* c) const {
  ...
  int cy1 = cy0 + 2;
  if (y1 >= cy1) cy1 = (y1 < hiEnd + 1) ? y1 : hiEnd;
...
bool gpu_env_is_pal(Core* c) { return c->mem_r8(kLibgpuVideoMode) != 0; }
