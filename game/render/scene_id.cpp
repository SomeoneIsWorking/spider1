...
SceneName::SceneName() {
  for (int i = 0; i < kBytes; ++i) { mRaw[i] = 0; mText[i] = '.'; }
  mText[kBytes] = '\0';
  mPrintable = false;
}
...
uint32_t SceneName::code() const {
  ...
  uint32_t level;
  if (c0 == 'd' || c0 == 'D')            level = kDemoLevel;
  else if (c1 - kAsciiZero  < 10u)       level = c1 - kAsciiZero;    // '0'..'9'
  else if (c1 - uint32_t('A') < 26u)     level = c1 - kLetterBias;   // 'A'..'Z'
  else if (c1 - uint32_t('a') < 26u)     level = c1 - kLowerBias;    // 'a'..'z'
  else                                   level = c1;                 // unfolded, as the guest leaves it
