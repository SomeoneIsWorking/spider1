...
static void spiderman_stgetnext(Core *c) {
  ...
  //
  // Two opposite faults look identical from outside: the PRODUCER never marking slots ready, and
  // the CONSUMER reading the wrong slot. They need opposite fixes, and guessing between them has
  // already cost several attempts. The slot status words settle it.
  //
  ...
  if (c->r[2] != 0 && lucent::channel_on("ring")) {
    ...
    if ((n++ % 200000) == 0) {
      ...
      lucent::debug("ring",
                    "base=0x{:08X} slots={} frameStart={} cons={} writeIdx={} | status: {}",
                    base,
                    slots,
                    c->mem_r32(kRingFrameStart),
                    c->mem_r32(kRingConsIdx),
                    c->mem_r32(kRingWriteIdx),
                    status.view());
    }
  }
}
