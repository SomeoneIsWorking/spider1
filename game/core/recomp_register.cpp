...
static const RecompRegistry g_spiderman_recomp = {
    ...
    // none, on the strength of the ISO tree carrying no overlay FILES. That was true and
    // misleading: the game loads further CODE at runtime out of CD.WAD as <name>.bin + <name>.rel
    // pairs, which tools/extract_modules.py now relocates offline so the recompiler can emit them
    // (RE-09). The table below is populated (currently: SHELL). The two overlay-specific setters
    // stay null because they are for games whose overlays share a FIXED slot and need per-slot
    // override installation; these modules load to distinct heap addresses and are routed by
...
void spiderman_install_recomp() {
  psxport_install_recomp(&g_spiderman_recomp);
}
