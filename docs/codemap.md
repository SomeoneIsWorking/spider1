...
| Scheduler | — | **missing** — the SDK task model may not apply to this engine, and RE-19's straight-line frame loop supports that: no task table is visible (RE-13). **No longer the blocker for a scene classifier**: RE-23 found the game's own level-name lens (`0x800A568C` + encoder `FUN_8005A734`, consumed by `FUN_80062CE0`'s per-frame switch over 0x201..0x803). C026's module-registry census stands and is still true; it was the wrong source to generalise from. What is still missing here is the WITHIN-level substate |
...
| Native renderer (`pc_render`) + scene classifier | `game/render/render_seam.cpp` `renderScene()` / `abortUnimplemented()` | **skeleton lands, ZERO producers — every scene aborts, and that is the correct state (RE-18 `re-partial`).** `PSXPORT_RENDER_PSX=0` aborts at submitFrame call #1 / frame 2 naming the scene, the DB lens (`db=8009A75C ot=800CA5F4 otHead=800CE5F0 pool=800E5604` — the exact addresses RE-12 measured with `PSXPORT_WWATCH`) and `geomValid=0` (the first submit is a boot-INIT submit, before any viewport). No OT-walk fallback, no escape hatch on that leg. **The DEFAULT is the reference leg**, set before `native_boot_run()` reads `PSXPORT_RENDER_PSX` so an explicit env value still wins — the opposite of Tomba!2's default, because this port has no producers and a `pc_render` default would abort every run including every boot gate. It flips WITH the first producer. **What is missing is RE-21**: the abort names the SCENE but not the unhandled display-object TYPE, because no type is decoded yet, so today the crash list is a backlog of length one |
