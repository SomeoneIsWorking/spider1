...
| C++ policy verifier | `.clang-format`, `.clang-tidy`, CTest `cpp_policy`, shared `external/psxport/tools/check_cpp_style.py` | **done 2026-08-21** — `ctest --test-dir build --output-on-failure -R cpp_policy` checks all 39 first-party source files non-mutatingly for format, runs clang-tidy on all 25 compile-backed first-party C++ translation units against the real Clang commands, and enforces the 1,200-line ownership cap. Generated/framework/vendor sources are outside this game repo's ownership. No pre-commit hook |
...
| **Whole guest-frame fallback for unported graphics** | `game/render/guest_frame_fallback.cpp`, `game/render/render_seam.cpp`; CTest `guest_frame_fallback` | **HACK-03 — explicit debt, not a producer.** For a named scene with no complete native producer, Native may super-call the unmodified retail `FUN_80061308` under a temporary `RenderPath::Gte`. That makes the submission a mutually-exclusive whole guest frame: the native envelope and native geometry are skipped, the exact guest-built OT/GTE result is consumed by the existing DrawOTag path, the PC rasterizer is used, and native enhancements are disabled during submission. The production decision rejects `nativeSubmissionStarted`, and `PSXPORT_FPS60=1` is refused as `INTERPOLATION_FORBIDDEN`; guest output is never interpolated. Final `3418a79b` live evidence: explicit Native submitted eight named-scene frames across `dem1 -> l1a1`, every one with `nativeSubmitted=0`, `nativeEnvelopeDelta=0`, and `interpolation=0`; `PSXPORT_SPIDER1_GUEST_FRAME_FALLBACK=0` and FPS60 controls retain the named opposite answers. Remove this row per scene as RE-21's real producer lands. |
...
| The framework itself | `external/psxport/` | **not this repo's subsystem** — a workspace symlink or private checkout with its own history and codemap. Framework changes land in that repo; this consumer was clean-configured and built with Clang 22.1.8, policy-verified, and bounded-live-verified at `3418a79b` on 2026-08-21, recorded in `psxport.pin`. The explicit Native fallback and zero-argument default-Gte routes both reached `dem1 -> l1a1`; each then stopped at the separately catalogued FPS60 queue-overflow boundary |
