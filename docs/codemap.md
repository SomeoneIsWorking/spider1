...
| Native frame loop / OT / packet pool | — | **missing, and its reach is now MEASURED** — that `GameConfig` group is deliberately zero; Phase 0 runs the guest's loop instead (RE-12). `spiderman_bootInit` `rec_dispatch`es the guest's own `main()` 0x8002C354 and never returns, so the framework's loop in `game_main` (`native_boot.cpp:452`) never starts: the unconditional `lucent::info` "entering native frame loop" (`native_boot.cpp:295`) printed **0 times** over a 230 s windowed run reaching present 13757, while the same line DOES print in Tomba2Engine runs (C025, issue 0011). Everything downstream of that loop — `native_step_frame`, `hooks->drawOTag`, `hooks->frameUpdate`, `PSXPORT_DEBUG=perf` — is dead in this port |
| Scheduler | — | **missing** — the SDK task model may not apply to this engine (RE-13). This is the blocker for a scene classifier: nothing here has a stage pointer, a substate selector or a task-slot layout (C026) |
| Native renderer (`pc_render`) + scene classifier | — | **missing, and BLOCKED — do not build the skeleton yet (RE-18, issue 0011).** Tomba!2's shape (`Engine::drawOTag` two-branch + `Render::classifyScene` + `abortUnimplemented`) does **not** transfer here on two independent measured counts: (1) the `drawOTag` hook is unreachable (C025, row above); (2) there is nothing to classify on. Tomba!2 keys `classifyScene` on five RE'd guest fields (`0x801FE000` task0 state, `0x801FE00C` stage ptr, `0x1F800138` task-sm, `0x801FE048` substate, `0x80109450` overlay sig) and this port has a counterpart for none of them. Its ONLY RE'd scene identity is the guest module registry, and a full load+evict census (`PSXPORT_DEBUG=ovload`, which logs both halves) over 13757 presents found **8 events / 3 names, all before present 780** — leaving 94.3% of the run on one constant set `{THUG, BLACKCAT}` that spans the attract fly-through *and* live 3D gameplay with the HUD up (C026). A classifier built on it would give the attract demo and gameplay the same name. Order: **RE-13, then RE-12, then RE-18** |
