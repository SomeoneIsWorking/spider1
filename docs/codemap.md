...
| Mesh submission boundary / render walk | guest `FUN_8002BD5C`, world render `FUN_80075D0C`, probe `game/render/mesh_probe.cpp`, list heads `0x800B5238` / `0x800B5234` | **RE-partial (RE-21, I031, C037)** — the frame phases and typed display-object walks remain mapped; `FUN_80077D64` now decodes a concrete mesh header: u16 vertex/secondary/face counts at +2/+4/+6, source vertices at +0x1C in 8-byte records, then 8-byte secondary records, then the variable face stream passed to `FUN_8007C4D8`. The observe-only probe super-calls every guest body and measured 27,579 contextual face-builder calls while sampling at least 64 object/mesh pairs (the roster cap), with 0 derived-layout mismatches; its control rejects a +4-byte face-stream perturbation. **Still missing:** face-record flags/index/UV/material semantics, texture source, the conditional object/matrix path, and classification of `FUN_8007C4D8` calls from other builders. There is still no native display-list producer |
