...
| C++ policy verifier | `.clang-format`, `.clang-tidy`, CTest `cpp_policy`, shared `external/psxport/tools/check_cpp_style.py` | **done 2026-08-21** — `ctest --test-dir build --output-on-failure -R cpp_policy` checks all first-party source non-mutatingly for format, runs clang-tidy on all 16 compile-backed first-party C++ translation units against the real Clang commands, and enforces the 1,200-line ownership cap. Generated/framework/vendor sources are outside this game repo's ownership. No pre-commit hook |
...
| Mesh submission boundary / render walk | guest `FUN_8002BD5C`, world render `FUN_80075D0C`, format owner `game/render/mesh_face_format.cpp`, probe `game/render/mesh_probe.cpp`, list heads `0x800B5238` / `0x800B5234` | **RE-partial (RE-21, I031/I033, C037/C039)** — `FUN_80077D64`'s header layout remains live-verified across 27,579 contextual calls with 0 mismatches. Exact executable instructions now decode the effective face header, byte stride, triangle/quad bit, direct-texture bit, source indices, and FT4 UV/CLUT/TPAGE fields. The first live contextual face is a 28-byte direct-textured quad with four in-range source vertices, UVs `[(59,253),(52,253),(59,246),(52,246)]`, CLUT `0x00E2` at `(544,3)`, and stored TPAGE `0x0008`; face flag `0x80` combines with it into effective 4-bpp TPAGE `0x0028` at `(512,0)`, blend mode 1. The observe-only probe reads only fields proven present by the decoded stride and still super-calls every guest body. **Still missing before a real producer:** the conditional object/matrix path and its transformation contract, the raw texture asset owner/lifetime, face variants beyond this first type, and attribution of `FUN_8007C4D8` calls from other builders. There is still no native display-list producer |
