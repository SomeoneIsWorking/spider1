...
| Disc provisioning + static recompilation | `tools/ensure_recomp.py`, `game/recomp_seeds.json` | **done (C042)** — hash-gated; MAIN + **30 runtime-loaded modules**. The resident graph is binary-discovered except for one measured BIOS continuation: `HookEntryInt` stores CdInit's post-`setjmp` PC `0x8008B990`, so current psxport requires that interior address under `main_reentry`. The `692b9b20` corpus emits 1,672 fragments from 738 discovery roots; `main` remains empty and no ordinary function entry is guessed |
...
| C++ policy verifier | `.clang-format`, `.clang-tidy`, CTest `cpp_policy`, shared `external/psxport/tools/check_cpp_style.py` | **done 2026-08-21** — `ctest --test-dir build --output-on-failure -R cpp_policy` checks all 33 first-party source files non-mutatingly for format, runs clang-tidy on all 21 compile-backed first-party C++ translation units against the real Clang commands, and enforces the 1,200-line ownership cap. Generated/framework/vendor sources are outside this game repo's ownership. No pre-commit hook |
...
| Mesh submission boundary / render walk | guest `FUN_8002BD5C`, direct chain `FUN_80076480 -> FUN_80077D64 -> FUN_8007C2AC`, source owners `game/render/mesh_face_format.cpp` / `mesh_transform.cpp`, authored upload/lifetime owner `game/render/asset_upload_ledger.cpp` + `texture_asset_probe.cpp`, observe-only `mesh_probe.cpp`, both-answer tests `mesh_transform_contract_test` / `asset_upload_ledger_test` | **RE-partial (RE-21, I031/I033/I034/I035, C037/C039/C040/C041)** — the first live 28-byte FT4 and its direct transform are instruction-exact and live-validated. **Authored source/lifetime resolved:** the mesh is retained at `Dem1_G.psx + 0xB4`; the exact texture target `(525,246 2x8)` and CLUT `(544,3 16x1)` were last uploaded by the retail parser for `Dem1_L.psx`, from raw offsets `0x2B30` and `0x38`. `FUN_80068BB0` shrinks that allocation from 12,288 to 48 bytes before the face is submitted, and `henchman.psx` immediately reuses the released range, so both raw upload sources already name different asset bytes while the `Dem1_L` slot stays live; the mesh allocation retains 7,836 bytes and remains live. The probe never consumes the OT, GTE registers, scratchpad projection, GTE output, or rendered VRAM. **Still missing before a real producer:** design the fail-fast producer around the retained mesh plus load-time texture ownership, then BREAK-FIRST gate it. Rotated/scaled objects, source flag `0x10`, other face variants, and other builders remain later corpus-driven backlog. There is still no native display-list producer. |
...
| The framework itself | `external/psxport/` | **not this repo's subsystem** — a workspace symlink or private checkout with its own history and codemap. Framework changes land in that repo; this consumer was Clang-built, policy-verified, and bounded-live-verified at `692b9b20` on 2026-08-21, recorded in `psxport.pin` |
