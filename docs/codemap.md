...
| Relocation-model gate | `tools/check_reloc_model.py` | **done** — checks the shape assumptions base-relative emission rests on across all 30 modules (8883 sites); self-tested (`--selftest`), refuses rather than returning empty on a missing corpus (INST-005) |
...
| FMV / intro movies | framework `dma_irq.h`/`mem.cpp`/`hle.cpp` + `tools/recomp/emit.py`; guest libstr + `FUN_8002AA0C` (player), `FUN_8002A338` (DecDCTvlc), `0x8002B28C` (MDEC-out callback) | **works** (RE-07, CLAIM-C019, verified on real data). Both logo movies decode and render: f120 = 99.95% non-black / 11395 colours (ACTIVISION), f300 = 25.70% / 8773 (Neversoft), menu unchanged at 99.44%. Three causes, all fixed and all in the FRAMEWORK, not the game: the DICR gate (C017), the recompiler unwinding DecDCTvlc after one block (RE-16), and DMA completions being announced on channel 3 only. Not measured: audio (every run used PSXPORT_NOAUDIO=1) and frame PACING against the movie's real duration. The TTSLOGO the boot probes is legitimately absent from the disc and the guest handles it |
