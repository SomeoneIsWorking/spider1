...
| Runtime image provisioning | Extract and authenticate the selected executable and runtime modules without emitting guest bodies | target launcher/provisioner beneath `tools/`; title manifests beneath `titles/` remain fact authority | target runtime-image provision entry | `docs/migration.md` |
| PSX guest executor | Per-`Core` Lightrec ownership, CPU/device synchronization, bounded exits, block cache, and invalidation | target `external/psxport/runtime/` owner; no title-local executor | target framework executor interface | `docs/migration.md` |
| Runtime dispatch | Image-aware native overrides, scoped original calls through Lightrec, and invalidation when dispatch policy changes | target framework runtime dispatch plus title-owned registration in `titles/<title>/` | target executor call boundary | `docs/migration.md` |
| Product targets | One executable per title, each consuming only its authenticated runtime image and title policy | `CMakeLists.txt`, `cmake/spiderman_port.cmake`, `titles/spiderman2/port.cmake` | CMake title selection loop | `docs/migration.md` |
...
| Frame cadence | Spider-Man 1 fields, callback delivery, pad/audio ordering, GPU-DMA timeout deadlines, resumable runtime STR/stream/post-logo exits, repeated-field pacing, and one-fence enforcement | current native owners in `titles/spiderman1/spider1_frame_driver.*`, `spider1_movie_driver.h`, and `spider1_stream_driver.h`; target executor-exit integration replaces the build-derived movie body | `Spider1FrameDriver::stepFrame`, target Lightrec resume boundary | `docs/migration.md` |
...
| Render orchestration | Scene ownership decision, frame envelope composition, and scoped original guest call through the runtime executor | `game/render/render_seam.*` plus target executor dispatch | `spiderman_install_render_seam` | `docs/re-frontier.md` |
...
| Offline-translation retirement | Delete the generator, emitted corpus, emission-only seeds, generated dispatch/tests, and provisioning hooks after representative gameplay passes | current `generated/`, `tools/ensure_recomp.py`, `recomp_seeds.json`, and generated-symbol wiring; target is absence | migration landing step | `docs/migration.md` |
