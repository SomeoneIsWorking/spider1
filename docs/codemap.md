...
| C++ policy verifier | `.clang-format`, `.clang-tidy`, CTest `cpp_policy`, shared `external/psxport/tools/check_cpp_style.py` | **done 2026-08-21** — `ctest --test-dir build --output-on-failure -R cpp_policy` checks all 28 first-party source files non-mutatingly for format, runs clang-tidy on all 18 compile-backed first-party C++ translation units against the real Clang commands, and enforces the 1,200-line ownership cap. Generated/framework/vendor sources are outside this game repo's ownership. No pre-commit hook |
...
| Mesh submission boundary / render walk | guest `FUN_8002BD5C`, direct chain `FUN_80076480 -> FUN_80077D64 -> FUN_8007C2AC`, source owners `game/render/mesh_face_format.cpp` / `mesh_transform.cpp`, observe-only `mesh_probe.cpp`, both-answer `tests/mesh_transform_contract_test.cpp` | **RE-partial (RE-21, I031/I033/I034, C037/C039/C040)** — the exact executable-derived face decoder covers header stride/flags/indices and the first FT4 UV/CLUT/TPAGE record. The first live face is a 28-byte direct-textured quad with four in-range object-local vertices and effective 4-bpp TPAGE `0x0028`. The direct object transform is now instruction-exact and live-validated: camera rotation comes from `camera+0x74`, translation is `(objectPosition20p12 sra 12)-cameraPosition`, and the two `FUN_80077D64` calls require zero rotation and clear scale flag `0x0200`; two distinct live tuples matched. **Attribution correction:** `0x8018BB90/0x9000` is the outer list head, not the owner; actual first owner `0x8018BBB4` has flags `0x0000`. The probe never consumes the OT, GTE registers, scratchpad projection, or GTE output. **Still missing before a real producer:** trace this face's raw texture/CLUT upload to its authored asset owner/lifetime; rotated/scaled objects, source flag `0x10`, other face variants, and other builders remain later corpus-driven backlog. There is still no native display-list producer. |
