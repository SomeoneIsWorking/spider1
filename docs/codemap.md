...
| FMV / intro movies | framework `dma_irq.h`/`mem.cpp`; guest libstr + `FUN_8002AA0C` | **partial** (RE-07). The STR sector ring no longer deadlocks: DPCR/DICR are modelled and the DMA3 completion callback fires once per FRAME instead of once per sector (CLAIM-C017). But the movies still do not PLAY — the player loop runs exactly one DecDCTin/DecDCTout per movie and abandons it (CLAIM-C018), and the intro display area is 512x1 so a decoded frame would have nowhere to land (issue 0002). The TTSLOGO the boot probes is legitimately absent from the disc and the guest handles it (pre-scan stores LBA 0) |
