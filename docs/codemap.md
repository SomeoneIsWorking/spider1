...
| Disc provisioning + static recompilation | `tools/ensure_recomp.py`, `game/recomp_seeds.json` | **done (C042)** — hash-gated; MAIN + **30 runtime-loaded modules**. The resident graph is binary-discovered except for one measured BIOS continuation: `HookEntryInt` stores CdInit's post-`setjmp` PC `0x8008B990`, so current psxport requires that interior address under `main_reentry`. The corpus verified current under `3418a79b` contains 1,672 resident fragments from 738 discovery roots; `main` remains empty and no ordinary function entry is guessed |
...
| **THE RUN GATE** — does the built binary still boot? | `tools/gate.py` | **done, extended 2026-08-22** (issues 0014/0017). `python3 tools/gate.py boot` launches `scratch/bin/spiderman_port` headless under `gpuguard run`, capped, and asserts on the port's OWN log lines: boot exe loaded, guest main dispatched, render seam installed AND fired, counters ADVANCED at ≥35% of the recorded baseline, ≥2 named-scene changes, and no failure pattern. It cannot use the REPL because this port never enters the framework frame loop that services it. `--selftest` now judges one known-good capture plus 17 broken variants (12 FAIL, 4 REFUSE, 1 GPU-device-loss STOP), including the exact `Fps60::rq_capture OVERFLOW` from issue 0017; `check-log <path>` runs the same analyser over captured output. Refusals exit 2, device loss exits 3. The launcher hang path is separately exercised for non-empty stderr capture and no surviving grandchild. |
| C++ policy verifier | `.clang-format`, `.clang-tidy`, CTest `cpp_policy`, shared `external/psxport/tools/check_cpp_style.py` | **done 2026-08-22** — `ctest --test-dir build --output-on-failure -R cpp_policy` checks all 41 first-party source files non-mutatingly for format, runs clang-tidy on all 26 compile-backed first-party C++ translation units against the real Clang commands, and enforces the 1,200-line ownership cap. Generated/framework/vendor sources are outside this game repo's ownership. No pre-commit hook |
...
| The framework itself | `external/psxport/` | **not this repo's subsystem** — a workspace symlink or private checkout with its own history and codemap. This consumer was clean-configured, Clang-built, policy-verified, and live-verified against recorded pin `3418a79b` on 2026-08-22. The shared queue behaved as designed once this guest-loop consumer supplied its own frame fence; no framework edit or pin bump was required. The live workspace framework remains independently dirty/incompatible and was not modified. |
