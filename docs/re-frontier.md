...
- where: guest: render walk FUN_8002BD5C; world render + projection setter FUN_80075D0C; direct object/mesh chain FUN_80076480 -> FUN_80077D64 -> FUN_8007C2AC; load-time mesh cook FUN_80068BB0 -> FUN_80074C98; camera pointer gp+0x1128, camera position +0x04/+0x08/+0x0C, camera rotation +0x74..+0x84; object flags +0x00, 20.12 position +0x04/+0x08/+0x0C, rotation +0x10/+0x12/+0x14; game/render/mesh_face_format.cpp, mesh_transform.cpp, mesh_asset_cook.cpp, asset_upload_ledger.cpp and texture_asset_probe.cpp; observe-only game/render/mesh_probe.cpp. Retail asset lifetime: FUN_80069A60 load, FUN_80068BB0 parse/trim, FUN_800695D0 unload, registry 0x800A0904 (0x40-byte slots, allocation at +0x14), LoadImage FUN_80081C50. Other per-object renderers FUN_80076FC0 / FUN_800762D4 / FUN_8007A764 / FUN_8007ABDC; display-object list heads 0x800B5238 and 0x800B5234 (next +0x1C, type +0x34, class +0x3C).
- gap: The first 28-byte direct-textured quad's source record, zero-rotation/unscaled object-to-camera transform, authored geometry/texture/CLUT ownership, and exact load-time-cooked retained record/lifetime are decoded; no producer exists yet. The next dependency is the first face's source-to-screen, cull, and lighting/colour contract: its `0x1000` flag enters the later FUN_8007C4D8 lighting path, so projected XY/depth/colour are not yet proven inputs to a native draw. Raw `Dem1_L.psx` upload pointers, census, OT packets, rendered VRAM, scratchpad projection, GTE registers, and GTE output remain forbidden inputs to a NATIVE producer. The user separately authorized actual guest-time GTE/OT output as a NON-INTERPOLATED fallback for graphics whose producer remains unported; that whole-frame debt is HACK-03, does not advance this step, and must remain auditable and mutually exclusive with native draws. Source-vertex flag `0x10`, rotated/scaled objects, unobserved face variants, and most FUN_8007C4D8 calls (201,541 of 229,120 in the long measurement) remain separate corpus-driven backlog.
...
- status: hack
- deps: RE-20, RE-21
- evidence: User-authorized debt path, 2026-08-21. Native scene selection happens before any native draw. For an unported named scene, the seam super-calls the unmodified retail FUN_80061308 under temporary RenderPath::Gte, so ResetGraph/PutDispEnv/PutDrawEnv/DrawOTag consume the actual guest-built frame and the PC rasterizer draws it with native enhancements disabled. Live Native run reached dem1 and l1a1; the first named frame logged nativeSubmitted=0, interpolation=0 and SUBMITTED. Forced-off run reached the same dem1 boundary and failed with DISABLED. PSXPORT_FPS60=1 reached the same boundary and failed with INTERPOLATION_FORBIDDEN. The production decision test also proves NATIVE_OVERLAP_FORBIDDEN and mode restoration.
