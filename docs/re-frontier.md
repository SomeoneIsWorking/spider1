...
- evidence: 
- where: 
- gap: STILL NOTHING RE'd for the SCHEDULER TASK LAYOUT itself — no task-slot table, no current-task pointer, no substate selector. What HAS changed 2026-08-06: this step is no longer the thing blocking a native-renderer scene classifier. RE-23 found the game's own scene identity (the level-name string at 0x800A568C and its encoder FUN_8005A734, consumed by FUN_80062CE0's per-frame switch over 0x201..0x803), which is the datum RE-18 actually needed. The 2026-08-06 measurement that stood here — the guest module registry being a near-useless discriminator (8 load/evict events, 3 names, all before present 780; 94.3% of a 13757-present run on one constant resident set spanning the attract fly-through AND live gameplay with the HUD up) — is UNCHANGED AND STILL TRUE; it was just the wrong source to generalise from. What remains genuinely un-RE'd and belongs to this step is the WITHIN-LEVEL substate (front-end page, cutscene vs play) and the task model itself.
- notes: docs/codemap.md records 'the SDK task model may not apply to this engine' — and the frame loop RE'd in RE-19 supports that: FUN_8002C174 is a straight-line per-frame loop with fixed phases and no visible task table, so do not go looking for Tomba!2's cooperative scheduler shape. RE-18 no longer waits on this step; it waits on RE-20 (the reachable seam) and RE-21 (producers).
...
- evidence: THE SKELETON EXISTS AND FAILS FAST, 2026-08-06. The seam is live (RE-20) and the scene identity is live (RE-23), so the two 2026-08-06 blockers are gone in fact and not just on paper. MEASURED: PSXPORT_RENDER_PSX=0, headless, PSXPORT_NOPACE=1 -> the run aborts at submitFrame call #1 / frame 2 with 'unimplemented native rendering: no native producer exists for ANY scene yet (RE-21 is unstarted)', printing the scene name and code, the encoder's address and consumer, the frame and call number, the full DB lens (db/drawenv/dispenv/ot/otHead/pool) and the projection state (geomValid/OFX/OFY/H). There is no OT-walk fallback and no env escape hatch on that leg.
...
- gap: WHAT IS MISSING IS THE PRODUCERS, i.e. RE-21 — renderScene() has no cases because there is nothing to dispatch TO. Two further honest gaps in the abort itself. (1) It names the SCENE but not the unhandled DISPLAY-OBJECT TYPE, because no display-object type has been decoded yet (RE-21); until it does, the crash list is a backlog of length one, which is exactly what this step's own text warned about. (2) It has only ever fired at submitFrame call #1, which RE-19 shows is a boot-INIT submit rather than an engine-rendered frame — so what the abort prints on a real gameplay frame is UNOBSERVED. Getting past call #1 requires a producer, so these two clear together.
...
- gap: NOT DONE. Nothing here is RE'd below the call-graph level: no object struct is decoded, no type code is named, no vertex/texture source is identified. This is the step that actually costs the work — everything else in the chain is plumbing. It must be done type by type, each with a fail-fast for the types not yet handled (RE-18), and each verified BREAK-FIRST: suppress the guest submit (RE-20), let the layer be honestly absent, then rebuild it from the object's own world state projected with the native camera. NEVER by reading the OT, GP0 packets or GTE registers — that is the tap coord/PROTOCOL.md bans absolutely.
