...
- gap: THE MOVIES STILL DO NOT PLAY — a SECOND, independent blocker (claim C018). The player FUN_8002AA0C (gp=0x800B47F4) runs exactly ONE loop iteration per movie: DecDCTin 0x80085B24 at 0x8002AF20 + DecDCTout 0x80085BA0 at 0x8002AF58, then branches back to the frame wait at 0x8002AD38 and leaves via a goto to LAB_8002AF90 — NOT the normal break (fntrace: 0x8002B18C never reached over a whole run). Two of the four goto paths are ruled out by watchpoint over a whole run: gp+0x68C (0x800B4E80, abort flag) — every store 0; DAT_800A4ED5 (pad skip) — every store 0. The two left are the frame-wait timeout and the dsx wait (gp+0x6D4 vs gp+0x6C2), both counting from 0x800000; FUN_8008710C(s_dsx___d, ...) is the guest's own printf on the dsx path and is the cheapest next discriminator. A THIRD gate sits behind that: the intro display area is 512x1 in this build, so a decoded movie has nowhere to land (issue 0002).
