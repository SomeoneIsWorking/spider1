...
- where: guest: render walk FUN_8002BD5C; world render + projection setter FUN_80075D0C; display-object walk FUN_80076480; dominant animated mesh path FUN_80077198 -> FUN_80077C08 -> FUN_8007C4D8; direct mesh path FUN_80077D64 -> FUN_8007C2AC -> FUN_8007C4D8; load-time mesh cook FUN_80068BB0 -> FUN_80074C98; camera pointer gp+0x1128, camera position +0x04/+0x08/+0x0C, camera rotation +0x74..+0x84; object flags +0x00, 20.12 position +0x04/+0x08/+0x0C, rotation +0x10/+0x12/+0x14; game/render/mesh_face_format.cpp, mesh_transform.cpp, face_builder_census.cpp, mesh_asset_cook.cpp, asset_upload_ledger.cpp and texture_asset_probe.cpp; observe-only game/render/mesh_probe.cpp. Retail asset lifetime: FUN_80069A60 load, FUN_80068BB0 parse/trim, FUN_800695D0 unload, registry 0x800A0904 (0x40-byte slots, allocation at +0x14), LoadImage FUN_80081C50. Other per-object renderers FUN_80076FC0 / FUN_800762D4 / FUN_8007A764 / FUN_8007ABDC; display-object list heads 0x800B5238 and 0x800B5234 (next +0x1C, type +0x34, class +0x3C).
- gap: No display-list producer exists yet. The former next step, implementing the first direct face as one FT4, was falsified by the caller/output census below: that 28-byte source record advances the primitive cursor by 480 bytes because FUN_8007C4D8 clips/expands it, while the direct FUN_80077D64 family owns only 593 of 24,576 calls in the bounded `dem1` corpus. The dependency order is now (1) decode FUN_80077C08's animated vertex transform, which owns 18,355 calls / 306,027 source faces in that corpus, then (2) decode FUN_8007C4D8's source-to-screen, cull, clip, lighting and colour contract, and only then (3) land a BREAK-FIRST producer. Raw `Dem1_L.psx` upload pointers, census output, OT packets, rendered VRAM, scratchpad projection, GTE registers, and GTE output remain forbidden inputs to a NATIVE producer. The user separately authorized actual guest-time GTE/OT output as a NON-INTERPOLATED fallback for graphics whose producer remains unported; that whole-frame debt is HACK-03, does not advance this step, and must remain auditable and mutually exclusive with native draws. Source-vertex flag `0x10`, other face variants, and the remaining statically enumerated builder families remain corpus-driven backlog.
