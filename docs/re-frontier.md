...
- where: guest: render walk FUN_8002BD5C; world render + projection setter FUN_80075D0C; direct object/mesh chain FUN_80076480 -> FUN_80077D64 -> FUN_8007C2AC; camera pointer gp+0x1128, camera position +0x04/+0x08/+0x0C, camera rotation +0x74..+0x84; object flags +0x00, 20.12 position +0x04/+0x08/+0x0C, rotation +0x10/+0x12/+0x14; game/render/mesh_face_format.cpp and game/render/mesh_transform.cpp; observe-only game/render/mesh_probe.cpp. Other per-object renderers FUN_80076FC0 / FUN_800762D4 / FUN_8007A764 / FUN_8007ABDC; display-object list heads 0x800B5238 and 0x800B5234 (next +0x1C, type +0x34, class +0x3C).
- gap: One 28-byte direct-textured quad source type and its zero-rotation/unscaled object-local-to-camera transform are decoded; no producer exists yet. The earlier `flags=0x9000 object` premise was falsified by correcting the probe boundary: 0x8018BB90/0x9000 is the outer list head, while actual owner 0x8018BBB4 has flags 0x0000. The earliest remaining source dependency for this first face is the raw texture asset owner/lifetime; do not guess it from CLUT/TPAGE output or read rendered VRAM back as producer input. Source-vertex flag 0x10, rotated/scaled objects, unobserved face variants, and most FUN_8007C4D8 calls (201,541 of 229,120 in the long measurement) remain separate backlog and must be decoded when the chosen producer corpus reaches them. Next: trace this face's texture/CLUT upload back to its authored asset owner and validate that source boundary live; only then can a fail-fast producer be designed and BREAK-FIRST tested. Do not consume the census, OT packets, scratchpad projection, GTE registers, or GTE output as producer input.
