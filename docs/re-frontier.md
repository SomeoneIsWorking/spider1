...
flush until `Fps60::frame_commit`. Spider-Man runs the guest loop, so no `native_step_frame` caller
owned that fence and captures crossed game frames until the 65,536-item fail-fast fired. The
game-local `game/render/guest_frame_commit.cpp` now maps the already-proven complete FUN_80061308
boundary to that fence after the retail body. Final Gte evidence reached 1,024 submissions through
