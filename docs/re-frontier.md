...
- notes: Issue #4 has the full write-up, with the superseded attribution kept visible. GameConfig::bootFmv is empty and PSXPORT_NO_FMV is a provable no-op for this port, so the framework's own FMV player is not involved; this is entirely the guest's path. **STATIC SKIP CONTRACT (C036; not runtime re-exercised):** `FUN_8006B514`/`FUN_8006B208` publish the Start up-to-down edge at `DAT_800A4ED5`; `FUN_8002AA0C` clears stale Start/Cross edges at entry, polls per decoded frame, deliberately clears them for the first 30 ticks, then sends either edge through its normal `LAB_8002AF90` full teardown. Its five static call instructions are accounted for by boot `FUN_8006BF9C` (IDs 0/1), queued-movie owner `FUN_8006BE28`, and overlay dispatcher `FUN_80010080` (IDs 3..23). Boot also tests the Start held byte `DAT_800A4ED4` after ID 0 to suppress ID 1. No alternate STR player surfaced. This does **not** establish skip behavior for non-STR in-engine sequences; their within-level state owner remains the RE-13 gap. No hack was added — the hack list stays empty. New instruments: PSXPORT_DEBUG=dmairq (I010) and the corrected PSXPORT_DEBUG=ring (I011, now sized by the guest's own slot count). fntrace_init() is now wired into game/core/game_hooks.cpp (it never was), so PSXPORT_FNTRACE works in this port.
