...
- gap: The first mesh container and its live object-relative translation are decoded, but the face-record flag/UV/material semantics are not, and the raw texture source is still unnamed. Most FUN_8007C4D8 calls do not come through FUN_80077D64 (201,541 of 229,120 in the long measurement), so their owning builders remain separate backlog rather than being forced through this mesh type. The object field at +0x38 is only conditionally a transform pointer (the first tuple carries 0x00010000), and the exact matrix source for the flags=0x9000 path still needs static RE. Next: decode one face record from mesh 0x8018BC38 back to its source vertices/UV/material, name that type, then build one fail-fast producer and BREAK-FIRST it. Do not consume the census, OT packets, scratchpad projection, or GTE registers as producer input.
