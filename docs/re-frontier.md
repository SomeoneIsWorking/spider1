...
- where: guest: render walk FUN_8002BD5C; world render + projection setter FUN_80075D0C; direct object/mesh chain FUN_80076480 -> FUN_80077D64 -> FUN_8007C2AC; camera pointer gp+0x1128, camera position +0x04/+0x08/+0x0C, camera rotation +0x74..+0x84; object flags +0x00, 20.12 position +0x04/+0x08/+0x0C, rotation +0x10/+0x12/+0x14; game/render/mesh_face_format.cpp, mesh_transform.cpp, asset_upload_ledger.cpp and texture_asset_probe.cpp; observe-only game/render/mesh_probe.cpp. Retail asset lifetime: FUN_80069A60 load, FUN_80068BB0 parse/trim, FUN_800695D0 unload, registry 0x800A0904 (0x40-byte slots, allocation at +0x14), LoadImage FUN_80081C50. Other per-object renderers FUN_80076FC0 / FUN_800762D4 / FUN_8007A764 / FUN_8007ABDC; display-object list heads 0x800B5238 and 0x800B5234 (next +0x1C, type +0x34, class +0x3C).
- gap: One 28-byte direct-textured quad source type, its zero-rotation/unscaled object-local-to-camera transform, and its authored geometry/texture/CLUT ownership and lifetime are decoded; no producer exists yet. The raw `Dem1_L.psx` upload sources are transient and already freed at render time, so a producer may not retain or rediscover those pointers; its texture ownership must be captured at the retail load/parse boundary. Next: design and BREAK-FIRST gate the first fail-fast producer from the retained `Dem1_G` mesh plus load-time texture ownership. Source-vertex flag `0x10`, rotated/scaled objects, unobserved face variants, and most FUN_8007C4D8 calls (201,541 of 229,120 in the long measurement) remain separate backlog. Do not consume the census, OT packets, rendered VRAM, scratchpad projection, GTE registers, or GTE output as producer input.
