...
- gap: One 28-byte direct-textured quad source type is decoded; no producer exists yet. The earliest remaining dependency is the conditional object/matrix path and the flags=0x9000 source-vertex transformation contract, because a producer cannot turn these object-local vertices into native positions without it. The raw texture asset owner/lifetime and unobserved face variants also remain unnamed. Most FUN_8007C4D8 calls do not come through FUN_80077D64 (201,541 of 229,120 in the long measurement), so their owning builders remain separate backlog rather than being forced through this mesh type. Next: instruction-exact RE of the FUN_80076480 -> FUN_80077D64 -> FUN_8007C2AC transform source and one live validation of its object-local-to-camera inputs; only then build one fail-fast producer and BREAK-FIRST it. Do not consume the census, OT packets, scratchpad projection, or GTE registers as producer input.
...
probe decoded object `0x8018BB90` / mesh `0x8018BC38`'s first face as a 28-byte direct-textured quad:
