...
def cmd_brief(a):
    ...
    # BOTH spellings, and a NEGATIVE that prints. Two independent silent-nothing bugs lived here:
    # (a) only `docs/code-map.md` was tried, so every project that spells it `docs/codemap.md` got the
    #     CODEMAP section skipped with no output at all — indistinguishable from "codemap has no match";
