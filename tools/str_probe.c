...
static int disc_open(const char* path) {
  if (chd_open(path, CHD_OPEN_READ, 0, &s_chd) != CHDERR_NONE) return 0;
...
static int str_parse(const uint8_t* raw2352, StrHeader* h) {
  ...
  h->magic = le16(d + 0);
  h->type  = le16(d + 2);
  if (h->magic != 0x0160 || h->type != 0x8001) return 0;
  h->chunk_no    = le16(d + 4);
  h->chunks      = le16(d + 6);
  h->frame_no    = le32(d + 8);
...
static int xa_freq(uint8_t coding)   { return (coding & 0x04) ? 18900 : 37800; }
static int xa_stereo(uint8_t coding) { return (coding & 0x01) ? 1 : 0; }
// Frames (sample-pairs for stereo, samples for mono) one Form2 audio sector decodes to.
static int xa_frames_per_sector(uint8_t coding) { return xa_stereo(coding) ? 2016 : 4032; }

static int selftest(void) {
  uint8_t s[2352]; StrHeader h; int fails = 0;
  // POSITIVE: a sector that MUST parse as frame 7, chunk 3 of 10, 320x240.
  memset(s, 0, sizeof s);
  uint8_t* d = s + 24;
  d[0]=0x60; d[1]=0x01; d[2]=0x01; d[3]=0x80;            // magic 0x0160, type 0x8001
  d[4]=3; d[5]=0; d[6]=10; d[7]=0;                        // chunk 3 of 10
  d[8]=7; d[9]=0; d[10]=0; d[11]=0;                       // frame 7
  d[16]=0x40; d[17]=0x01; d[18]=0xF0; d[19]=0x00;         // 320 x 240
  if (!str_parse(s, &h)) { printf("selftest FAIL: valid STR sector did not parse\n"); fails++; }
  else if (h.frame_no != 7 || h.chunks != 10 || h.chunk_no != 3 || h.width != 320 || h.height != 240) {
    printf("selftest FAIL: parsed wrong: frame=%u chunk=%u/%u %ux%u\n",
           h.frame_no, h.chunk_no, h.chunks, h.width, h.height); fails++;
  }
  // NEGATIVE: garbage must NOT parse. A parser that accepts everything reports 0 anomalies too.
  memset(s, 0xA5, sizeof s);
  if (str_parse(s, &h)) { printf("selftest FAIL: garbage sector parsed as STR\n"); fails++; }
  // NEGATIVE 2: right magic, wrong type — must still be rejected.
  memset(s, 0, sizeof s); d = s + 24; d[0]=0x60; d[1]=0x01; d[2]=0x99; d[3]=0x99;
  if (str_parse(s, &h)) { printf("selftest FAIL: magic-only sector parsed as STR\n"); fails++; }
  // XA rate decode, both classes.
  if (xa_freq(0x00) != 37800 || xa_freq(0x04) != 18900) { printf("selftest FAIL: xa_freq\n"); fails++; }
  if (xa_stereo(0x00) != 0   || xa_stereo(0x01) != 1)   { printf("selftest FAIL: xa_stereo\n"); fails++; }
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (!strcmp(argv[i], "--frames")) want_frames = 1;
  }
  if (argc < 4) {
    fprintf(stderr, "usage: str_probe <disc.chd> <start_lba> <sectors> [--frames]\n"
                    "       str_probe --selftest\n");
    return 2;
  }
  ...
  if (!disc_open(disc)) { fprintf(stderr, "cannot open CHD: %s\n", disc); return 1; }
  ...
  uint8_t  coding = 0xFF; uint32_t coding_changes = 0;
  ...
  for (uint32_t i = 0; i < n; i++) {
    ...
    if (!disc_read_raw(lba, raw)) {
      fprintf(stderr, "READ FAILED at LBA %u after %u sectors — counts below cover ONLY [%u..%u]\n",
              lba, read, lba0, lba ? lba - 1 : lba0);
      return 1;
    }
    read++;
    if (raw[15] != 2) { notmode2++; continue; }
    uint8_t submode = raw[18];
    if (submode & 0x04) {                       // Form2 XA audio
      audio++;
      if (!first_audio_lba) first_audio_lba = lba;
      last_audio_lba = lba;
      if (coding == 0xFF) coding = raw[19];
      else if (raw[19] != coding) coding_changes++;
      continue;
    }
    StrHeader s;
    if (str_parse(raw, &s)) {
      ...
      if (s.chunk_no == 0) {
        frames++;
        if (frames == 1) { frame_no_first = s.frame_no; w = s.width; h = s.height; }
        else if (s.width != w || s.height != h) size_changes++;
        frame_no_last = s.frame_no;
        if (s.chunks < chunks_min) chunks_min = s.chunks;
        if (s.chunks > chunks_max) chunks_max = s.chunks;
        if (want_frames)
          printf("  frame %-5u LBA %-8u chunks=%-3u bytes=%-7u %ux%u\n",
                 s.frame_no, lba, s.chunks, s.frame_bytes, s.width, s.height);
      }
    } else {
      other++;
    }
  ...
  printf("\nstr_probe %s  LBA [%u..%u], %u sectors requested, %u READ\n",
         disc, lba0, lba0 + n - 1, n, read);
  printf("  sector census : %u STR video, %u XA audio, %u other Mode2, %u not-Mode2  (sum %u)\n",
         video, audio, other, notmode2, video + audio + other + notmode2);
  ...
  if (!frames)
    ...
    printf("  VIDEO         : %u frames (frame_no %u..%u), %ux%u, chunks/frame %u..%u%s\n",
           frames, frame_no_first, frame_no_last, w, h, chunks_min, chunks_max,
    ...
    printf("  DURATION      : %.2f s at 2x (150 sectors/s) -> %.2f fps | %.2f s at 1x -> %.2f fps\n",
           secs2x, frames / secs2x, secs1x, frames / secs1x);
  }
  if (!audio)
    printf("  AUDIO         : NO XA audio sectors (submode bit2) in %u sectors read — this movie is "
    ...
    printf("  AUDIO         : %u XA sectors, LBA %u..%u, coding 0x%02X = %d Hz %s, "
           ...
           audio, first_audio_lba, last_audio_lba, coding, freq, st ? "stereo" : "mono", fps_, asecs,
           coding_changes ? "  [!! coding byte CHANGES mid-stream]" : "");
    printf("  AUDIO SAMPLES : %.0f frames at %d Hz = %.0f int16 values (%s)\n",
           (double)audio * fps_, freq, (double)audio * fps_ * (st ? 2 : 1), st ? "stereo" : "mono");
  }
  return 0;
}
